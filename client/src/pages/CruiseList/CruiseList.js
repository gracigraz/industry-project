...
function CruiseList() {
  ...
  function handleCompareClick() {
    let selectedButton = document.querySelectorAll(
      ".cruise-list__compare-button"
    );
    let selectedComparison = document.querySelectorAll(
      ".cruise-list__comparison-container"
    );

    if (selectedButton[0].innerHTML === "COMPARE") {
      selectedButton[0].innerHTML = "SELECT CRUISES TO COMPARE";

      let comparisonH2 = selectedComparison[0].appendChild(
    ...
    if (
      selectedButton[0].innerHTML === "SELECT CRUISES TO COMPARE" &&
      selectedCruises.length === 2
    ) {
      window.location.replace(
        "/compare/" + selectedCruises[0] + "/" + selectedCruises[1]
      );
    }
  }
  ...
  function handleSelectCruise(cruiseClick) {
    ...
    if (selectedButton[0].innerHTML === "SELECT CRUISES TO COMPARE") {
      if (selectedCruises.length === 0) {
        ...
        let comparisonH3 = selectedComparison[0].appendChild(
          document.createElement("h3")
        );
        comparisonH3.textContent = cruiseList[cruiseClick - 1].boat_name;
        let comparisonAnd = selectedComparison[0].appendChild(
          document.createElement("h3")
        );
  ...
  return (
    <div className="cruise-list">
      ...
      <section className="cruise-list__compare-container">
        <button
          className="cruise-list__compare-button"
          onClick={handleCompareClick}
        >
          COMPARE
        </button>
        <div className="cruise-list__comparison-container"></div>
      </section>
      {cruiseList.map((cruiseIteration) => {
        return (
          <div key={cruiseIteration.id}>
            <section
              onClick={() => {
                handleSelectCruise(cruiseIteration.id);
              }}
              className="cruise-list__section"
            >
